...
# Questions
questions = [f"1. What is the correct way to create an empty list?\n"
             f"   a) `list = []`\n   b) `list = '{{}}'`\n   c) `list = ()`\n   d) `list = ''`\n\n\n",
             f"2. Which method is used to add an element to the end of a list?\n   a) `append()`\n   b) `extend()`\n"
             f"   c) `insert()`\n   d) `remove()`\n\n\n"
             f"3. How do you access the third element of a list?\n   a) `list[0]`\n   b) `list[1]`\n   c) `list[2]`\n"
             f"   d) `list[3]`\n\n\n",
             f"4. What is the result of the following code?\n   "
             f"my_list = [1, 2, 3]\n   my_list.remove(2)\n   print(my_list)\n"
             f"   a) `[1, 2, 3]`\n   b) `[1, 3]`\n   c) `[2, 3]`\n   d) `Error`",
             f"5. How can you find the number of elements in a list?\n   a) `list.length()`\n   b) `list.count()`\n"
             f"   c) `list.size()`\n   d) `len(list)`",
             f"6. What method is used to sort a list in ascending order?\n   a) `list.sort()`\n   "
             f"b) `list.reverse()`\n   c) `list.append()`\n   d) `list.remove()`",
             f"7. What happens when you use the `+` operator to concatenate two lists?\n   a) The two lists are "
             f"combined into a single list.\n   b) The `+` operator cannot be used with lists.\n"
             f"   c) An error is thrown.\n   d) The lists remain unchanged."]
